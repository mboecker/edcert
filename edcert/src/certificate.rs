...
//            try!(s.emit_struct_field("public-key",
//                                     1,
//                                     |s| bytes_to_longs(&self.public_key).encode(s)));
//            if self.signature.is_some() {
//                try!(s.emit_struct_field("signature", 1, |s| self.signature.encode(s)));
//            } else {
//                try!(s.emit_struct_field("signature", 1, |s| (None as Option<u8>).encode(s)));
//            }
//            Ok(())
//        })
//    }
...
//    fn decode<S: Decoder>(s: &mut S) -> Result<Certificate, S::Error> {
//    	
//        s.read_struct("Certificate", 5, |s| {
//				
//			println!("Lese zertifikat");
//			
//			let values : BTreeMap<String, String> = try!(s.read_struct_field("meta", 0, |s| Decodable::decode(s)));
//			
//			println!("fertig mit meta");
//			
//			let expires : String = try!(s.read_struct_field("expires", 3, |s| Decodable::decode(s)));
//			let expires = chrono::DateTime::parse_from_rfc3339(&expires).expect("Failed to parse expires date").with_timezone(&chrono::UTC);
//			
//			let signature : Result<Signature, _> = s.read_struct_field("signature", 4, |s| Decodable::decode(s));
//			let signature = match signature {
//				Err(_) => {
//					println!("hat keine signatur...");
//					None
//				},
//				Ok(signature) => {
//					println!("hat signatur!");
//					Some(signature)
//				}
//			};
//			
//			println!("expires und signature fertig!");
//			
//			let r = Ok(Certificate {
//				meta: Meta::new(values),
//				public_key: Vec::new(), //longs_to_bytes(&try!(s.read_struct_field("public-key", 0, |s| Decodable::decode(s)))),
//				private_key: None, //try!(s.read_struct_field("private_key", 0, |s| Decodable::decode(s))),
//				expires: expires,
//				signature: signature,
//			});
//			
//			println!("fertig mit zertifikat!");
//			
//			r
//		})
//    }
